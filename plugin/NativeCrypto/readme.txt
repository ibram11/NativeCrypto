...
These methods wrap the slog2c() (https://developer.blackberry.com/native/reference/core/com.qnx.doc.neutrino.lib_ref/topic/s/slog2c.html) method which is the fastest logging method, but accepts only character arrays. The Logger creates two different sized buffers: One larger for high frequency, but lower priority messages, and one smaller for low frequency but higher priority messages. The methods above choose the appropriate buffer to use. If you need to send more complicated log messages use the slog2fa() (https://developer.blackberry.com/native/reference/core/com.qnx.doc.neutrino.lib_ref/topic/s/slog2fa.html) method, and you can access the buffers through these methods:
...
4. Click Okay to the build settings change.
5. When prompted choose where the library is that you want to add. For any platform library, you will want the first option.
6. Find the library in the list. In this example we've searched for and found the QtCore library
...
Note: Repeat these steps so the same settings exist on both the device and simulator build configurations
...
It is required for things like LED, Vibration, Audio and many others. See http://developer.blackberry.com/native/reference/bb10/com.qnx.doc.bps.lib_ref/com.qnx.doc.bps.lib_ref/topic/bps_initialize.html.
